...
describe('Modular Inverse (modinv)', () => {
  ...
  it('should work with negative inputs', () => {
    modInverse(-3, 10).should.be.equal(-7)
    modInverse(-63, 10).should.be.equal(-7)
    modInverse(-7, 10).should.be.equal(-3)
    modInverse(-27, 10).should.be.equal(-3)
    modInverse(-9, 13).should.be.equal(-3)
  })

  it('should not work if m < 2', () => {
    modInverse(3, 0).should.be.NaN
    modInverse(63, -1).should.be.NaN
    modInverse(7, 1).should.be.be.NaN
  })

  it('should be NaN if a % m == 0', () => {
    modInverse(5, 5).should.be.be.NaN
    modInverse(855, 5).should.be.be.NaN
    modInverse(-45, 5).should.be.be.NaN
  })

  it('should not work if gcd(a, m) != 1', () => {
    modInverse(8, 24).should.be.NaN
    modInverse(6, 12).should.be.NaN
    modInverse(8, 4).should.be.be.NaN
    modInverse(-8, 4).should.be.be.NaN
  })

  it('should pass this basic set', () => {
    modInverse(15, 1).should.be.be.NaN
    modInverse(0, 1).should.be.be.NaN
    modInverse(1, 1).should.be.NaN
    modInverse(-41, 1).should.be.NaN
    modInverse(-1, 1).should.be.NaN
